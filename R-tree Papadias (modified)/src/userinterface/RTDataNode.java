...
public final class RTDataNode extends RTNode implements Node
{
    ...
    public RTDataNode(RTree rt, RTDataNode a, int i)
    {
    	super(rt);
        ...
        dirty = true;
    	 ...
    	 data[0] = new Data(dimension);
    	 data[0].data = new float[2*Constants.DIMENSION];
    	 
    	 for(int j = 0; j < a.data[i].data.length; j++)
    		 data[0].data[j] = a.data[i].data[j];
    }
    ...
    /*
     * calls get_mbr() if res_mbr is null, else returns res_mbr
     * 
     */
    public float[] get_res_mbr()
    {
    	if(res_mbr == null)
    		return get_mbr();
    	else
